...
export const validCity = (city: string): string => {
	...
	if (city.length < 2) return 'Недопустимо использование меньше 2-х символов';

	if (city.length > 60)
		return 'Недопустимо использование больше 60-ти символов';

	if (!allowedChars.test(city)) return 'Используются недопустимые символы';
...
export const validFirstname = (firstname: string): string => {
	if (!firstname) return 'Поле является обязательным';
...
export const validLastname = (lastname: string): string => {
	if (!lastname) return 'Поле является обязательным';

	if (lastname.length < 2) return 'Недопустимо использование меньше 2 символов';
...
export const validOtp = (otp: string): string => {
	if (otp.length !== 6) {
